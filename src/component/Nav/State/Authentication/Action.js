...
export const registerterUser = (reqData)=>async(dispatch)=>{
    dispatch({type:REGISTER_REQUEST})
    try{
        const {data} = await axios.post(`${API_URL}/auth/signup` , reqData.userData)
        if(data.jwt)localStorage.setItem("jwt", data.jwt);
        if(data.role ==="ROLE_RESTAURANT-OWNER"){ // if you login as restaurant admin, it will take you to the admin/restaurant end point
